...
@section('contenido')
<form action="{{ route('career.update', $career->id) }}" method="POST" id="formEditarCareer"
    ...
    <div class="mb-3">
        ...
        <input type="text" name="name_career" id="name_career" class="form-control"
               value="{{ old('name_career', $career->name) }}" required>
    </div>

    <div class="mb-3">
        <label for="code_career" class="form-label fw-bold">Código:</label>
        <input type="text" name="code_career" id="code_career" class="form-control"
               value="{{ old('code_career', $career->code) }}" required>
    </div>
    ...
    <div class="mb-3">
        ...
        <select name="faculty_id_career" id="faculty_id_career" class="form-select" required>
            ...
            @foreach($faculties as $faculty)
                <option value="{{ $faculty->id }}" {{ $faculty->id == $career->faculty_id ? 'selected' : '' }}>
                    {{ $faculty->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="row">
        <div class="col-md-6 mb-3">
            ...
            <input type="number" name="duration_years_career" id="duration_years_career" class="form-control"
                   value="{{ old('duration_years_career', $career->duration_years) }}" min="1" max="10" required>
        </div>
        ...
        <div class="col-md-6 mb-3">
            ...
            <select name="level_career" id="level_career" class="form-select" required>
                ...
                <option value="pregrado" {{ $career->level == 'pregrado' ? 'selected' : '' }}>Pregrado</option>
                <option value="postgrado" {{ $career->level == 'postgrado' ? 'selected' : '' }}>Postgrado</option>
            </select>
        </div>
    </div>
    ...
    <div class="mb-3">
        ...
        <input type="text" name="degree_awarded_career" id="degree_awarded_career" class="form-control"
               value="{{ old('degree_awarded_career', $career->degree_awarded) }}" required>
    </div>
    ...
    <div class="mb-3">
        ...
        <select name="status_career" id="status_career" class="form-select" required>
            <option value="activo" {{ $career->status == 'activo' ? 'selected' : '' }}>Activo</option>
            <option value="inactivo" {{ $career->status == 'inactivo' ? 'selected' : '' }}>Inactivo</option>
        </select>
    </div>
